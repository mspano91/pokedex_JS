    ...
    .then((pokemon) => {
      contPoke.innerHTML = `
        <div class="pokemon-block">
                 ...
                 <h1 class="name">#${pokemon.id
                   .toString()
                   .padStart(4, 0)} - ${pokemon.name.toUpperCase()}</h1>
            ...
            <div class="pokemon-skills pokemon-block">
                    ...
                    <p class="skills"> Type: ${pokemon.types.map(
                      (el) => el.type.name
                    )}</p>
                    <p class="skills"> Ability: ${
...
formPoke.addEventListener("keyup", (event) => {
  ...
  const inputName = inputPoke.value.trim().toLowerCase(); // cConvertir a minúsculas y eliminar espacios al principio y al final
